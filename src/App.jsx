...
function App() {
  ...
  // very few books, check the list of all books at the bottom of the page and try searching for that one... 
  // Can search for 'the' and return a lot of books!
  ...
  async function deleteBookData(title) {
    try {
      const result = await fetch("", {});
    } catch (error) {}
  }
    ...
    <div className="App">
      <header className="App-header">
        ...
        >
          ...
          >
            <label
              style={{ fontSize: 16, width: "100%", textAlign: "start" }}
              htmlFor="searchSpecificBookInput"
            >
              Search:
            </label>
            <input
        ...
        {books.length > 0 ? (
          <div>
            <ul>
              ...
              >
                <label style={{ fontSize: 16 }} htmlFor="filterBooksInput">
                  Filter:
                </label>
                <input
                  style={{ fontSize: 16 }}
              ...
              {books.map((book, index) => {
                {
                  /* index is automatically genereted */
                }
                if (
                  filterBooksInput === "" ||
                  book.title
                    .toLowerCase()
                    .startsWith(filterBooksInput.toLowerCase())
                )
                  return (
                    <li style={{ fontSize: 12 }} key={index}>
                      <p>{book.title}</p>
          ...
          <div style={{ height: 200, width: "100%" }}>Loading books...</div>
        )}
      </header>
    </div>
  );
